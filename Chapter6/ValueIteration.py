# -*- coding: utf-8 -*-
"""
Created on Thu Jul  9 17:15:38 2020
...
from PoliceIteration import PolicyIteration


@contextmanager  # Python使用with上下文实现计时功能(https://www.jb51.net/article/136099.htm)
def timer(name):
    start = time.time()
    yield
    end = time.time()
...
class PolicyIterationWithTimer(PolicyIteration):  # 继承父类PolicyIteration
    def policy_iteration(self, agent, max_iter=-1):  # 重写方法policy_iteration
        iteration = 0
        while True:
            iteration += 1
...
class ValueIteration(object):  # 价值收敛的时候，策略也就收敛了
...
class GeneralizedIteration():
    def __init__(self):
        self.pi_algo = PolicyIteration()
        self.vi_algo = ValueIteration()

    def generalized_iteration(self, agent):
...
def generalized_iteration_demo():
    ...
    env = SnakeEnv(10, [3, 6])
    agent = TableAgent(env)
    ge_algo = GeneralizedIteration()
    ge_algo.generalized_iteration(agent)
    print('return_pi={}'.format(eval_game(env, agent)))
    print(agent.pi)
